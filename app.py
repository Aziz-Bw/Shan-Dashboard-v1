...
# --- CSS بسيط فقط للخط والاتجاه (بدون ألوان معقدة) ---
st.markdown("""
<style>
...
# --- 3. دوال المعالجة (النسخة المستقرة) ---
...
def load_sales_data(file_header, file_items):
    try:
        # إعادة مؤشر الملف للبداية (مهم جداً لعدم اختفاء البيانات)
        file_header.seek(0)
        file_items.seek(0)
...
# ========================================================
# الصفحة 1: المبيعات (التصميم الأصلي المستقر)
# ========================================================
if page == "💰 المبيعات والأرباح":
    ...
    if st.session_state['uploaded_files']:
        ...
        if df is not None:
            ...
            # عرض المؤشرات (Standard Streamlit Metrics)
            m1, m2, m3, m4, m5 = st.columns(5)
            m1.metric("صافي المبيعات", f"{sales:,.0f}")
            m2.metric("صافي الربح", f"{profit:,.0f}", f"{margin:.1f}%")
            m3.metric("تكلفة البضاعة", f"{cost:,.0f}")
            m4.metric("المرتجعات", f"{ret_val:,.0f}", f"عدد: {ret_count}")
            m5.metric("عدد الفواتير", f"{inv_count}")
            ...
            with g1:
                ...
                fig = px.bar(s_perf, x='SalesMan', y=['Amount', 'Profit'], barmode='group')
                st.plotly_chart(fig, use_container_width=True)
                
            with g2:
                st.subheader("تحليل الأصناف")
                top_items = df_sub.groupby('StockName')['Qty'].sum().reset_index().sort_values('Qty', ascending=False).head(10)
                st.dataframe(top_items, use_container_width=True, hide_index=True)

    else:
        st.warning("⚠️ الرجاء رفع ملفات المبيعات من القائمة الجانبية.")

# ========================================================
# الصفحة 2: التحصيل والديون (المنطق الذكي + استبعاد البنوك)
# ========================================================
elif page == "💸 التحصيل والديون":
    ...
    if st.session_state['ledger_file']:
        ...
        if df_ledger is not None:
            # --- 1. الكشف الذكي عن العملاء ---
            # نبحث عن الحسابات التي لديها عمليات "إيرادات مبيعات"
            if 'AcLedger' in df_ledger.columns:
                sales_transactions = df_ledger[df_ledger['AcLedger'].astype(str).str.contains("إيرادات المبيعات|مبيعات", na=False)]
                valid_customers_list = sales_transactions['LedgerName'].unique().tolist()
                
                # --- 2. الاستبعاد اليدوي (Blacklist) ---
                # هنا نستبعد البنوك وأي حسابات غير مرغوبة ظهرت بالخطأ
                ...
                if len(final_customers) > 0:
                    # --- 3. حساب الأرصدة ---
                    # نأخذ كامل العمليات لهؤلاء العملاء فقط
                    ...
                    # تصفية: نأخذ فقط من عليه دين (أكثر من 10 ريال)
                    ...
                    st.success(f"تم تحديد {count_debtors} عميل عليهم مديونيات قائمة.")
                    
                    k1, k2 = st.columns(2)
                    k1.metric("إجمالي الديون (في السوق)", f"{total_debt:,.0f} ر.س")
                    k2.metric("عدد العملاء المدينين", f"{count_debtors}")
                    ...
                    st.warning("لم يتم العثور على عملاء مطابقين.")
            else:
